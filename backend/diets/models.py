    ...
    for i, meal in enumerate(value):
        ...
        if not all(k in meal for k in ["nome", "horario", "alimentos"]):
            raise ValidationError(
                f"A refeição na posição {i} deve ter 'nome', 'horario' e 'alimentos'."
            )

        if not isinstance(meal["alimentos"], list):
            raise ValidationError(
                f"Os alimentos da refeição na posição {i} devem ser uma lista."
            )

        for j, food in enumerate(meal["alimentos"]):
            if not isinstance(food, dict):
                raise ValidationError(
                    f"O alimento na posição {j} da refeição na posição {i} deve ser um objeto."
                )
            ...
            if not all(k in food for k in ["nome", "quantidade", "unidade"]):
                raise ValidationError(
                    f"O alimento na posição {j} da refeição na posição {i} deve ter 'nome', 'quantidade' e 'unidade'."
                )
...
def validate_substitutions_schema(value):
    ...
    for i, sub in enumerate(value):
        ...
        if not all(k in sub for k in ["original", "options"]):
            raise ValidationError(
                f"A substituição na posição {i} deve ter 'original' e 'options'."
            )

        if not isinstance(sub["options"], list):
            raise ValidationError(
                f"As opções da substituição na posição {i} devem ser uma lista."
            )

        for j, option in enumerate(sub["options"]):
            if not isinstance(option, dict):
                raise ValidationError(
                    f"A opção na posição {j} da substituição na posição {i} deve ser um objeto."
                )
            ...
            if not all(k in option for k in ["name", "quantity"]):
                raise ValidationError(
                    f"A opção na posição {j} da substituição na posição {i} deve ter 'name' e 'quantity'."
                )
...
class AlimentoTACO(models.Model):
    ...
    codigo = models.CharField(
        max_length=10, unique=True, help_text="Código do alimento na TACO"
    )
    ...
    # Minerais
    calcio_mg = models.FloatField(
        null=True, blank=True, help_text="Cálcio em mg por 100g"
    )
    ferro_mg = models.FloatField(
        null=True, blank=True, help_text="Ferro em mg por 100g"
    )
    sodio_mg = models.FloatField(
        null=True, blank=True, help_text="Sódio em mg por 100g"
    )

    # Vitaminas
    vitamina_c_mg = models.FloatField(
        null=True, blank=True, help_text="Vitamina C em mg por 100g"
    )

    # Categorização
    grupo = models.CharField(
        max_length=50, help_text="Grupo alimentar (ex: Cereais, Carnes, etc.)"
    )
    ...
    unidade_caseira = models.CharField(
        ...
        null=True,
        blank=True,
        help_text="Unidade caseira (ex: 1 xícara, 1 fatia)",
    )
    peso_unidade_caseira_g = models.FloatField(
        null=True, blank=True, help_text="Peso em gramas da unidade caseira"
    )
...
class AlimentoTBCA(models.Model):
    ...
    codigo = models.CharField(
        max_length=20, unique=True, help_text="Código do alimento na TBCA"
    )
    ...
    lipidios_g = models.FloatField(help_text="Lipídios em gramas por 100g")
    carboidrato_g = models.FloatField(help_text="Carboidrato em gramas por 100g")
    fibra_g = models.FloatField(
        null=True, blank=True, help_text="Fibra em gramas por 100g"
    )
    ...
    vitamina_a_mcg = models.FloatField(
        null=True, blank=True, help_text="Vitamina A em mcg por 100g"
    )
    ...
    # Unidade caseira
    unidade_caseira = models.CharField(
        max_length=50, null=True, blank=True, help_text="Unidade caseira"
    )
    peso_unidade_caseira_g = models.FloatField(
        null=True, blank=True, help_text="Peso em gramas da unidade caseira"
    )
...
class AlimentoUSDA(models.Model):
    ...
    vitamina_d_mcg = models.FloatField(
        null=True, blank=True, help_text="Vitamina D em mcg por 100g"
    )
    ...
    # Porção padrão
    porcao_padrao_g = models.FloatField(
        default=100, help_text="Porção padrão em gramas"
    )
...
class Diet(models.Model):
    ...
    instructions = models.TextField(
        blank=True, null=True, help_text="Instruções e observações"
    )

    # Cálculos nutricionais (novos campos)
    tmb = models.IntegerField(null=True, blank=True, help_text="Taxa Metabólica Basal")
    gcdt = models.IntegerField(
        null=True, blank=True, help_text="Gasto Calórico Diário Total"
    )
    target_calories = models.IntegerField(
        null=True, blank=True, help_text="Meta de calorias"
    )
    ...
    diet_type = models.CharField(
        max_length=50,
        default="balanced",
        help_text="Tipo de dieta (balanced, lowcarb, etc)",
    )
    calculation_method = models.CharField(
        max_length=50,
        default="mifflin",
        help_text="Método de cálculo (mifflin, harris, etc)",
    )

    # Arquivo PDF gerado
    pdf_file = models.FileField(
        upload_to="diet_pdfs/%Y/%m/",
        null=True,
        blank=True,
        help_text="Arquivo PDF da dieta gerado",
    )
    ...
    target_protein = models.DecimalField(
        ...
        null=True,
        blank=True,
        help_text="Meta de proteínas em gramas",
    )
    target_carbs = models.DecimalField(
        ...
        null=True,
        blank=True,
        help_text="Meta de carboidratos em gramas",
    )
    target_fats = models.DecimalField(
        max_digits=6,
        decimal_places=2,
        null=True,
        blank=True,
        help_text="Meta de gorduras em gramas",
    )
    ...
    substitutions = models.JSONField(
        null=True, blank=True, validators=[validate_substitutions_schema]
    )
    notes = models.TextField(null=True, blank=True, verbose_name="Observações")
    meta_calorias = models.PositiveIntegerField(
        null=True, blank=True, help_text="Meta de calorias diárias (legado)"
    )
    meta_carboidratos = models.PositiveIntegerField(
        null=True, blank=True, help_text="Meta de carboidratos em gramas (legado)"
    )
    meta_proteinas = models.PositiveIntegerField(
        null=True, blank=True, help_text="Meta de proteínas em gramas (legado)"
    )
    meta_gorduras = models.PositiveIntegerField(
        null=True, blank=True, help_text="Meta de gorduras em gramas (legado)"
    )
    tmb_calculado = models.PositiveIntegerField(
        null=True, blank=True, help_text="Taxa Metabólica Basal calculada (legado)"
    )
    gcdt_calculado = models.PositiveIntegerField(
        null=True,
        blank=True,
        help_text="Gasto Calórico Diário Total calculado (legado)",
    )
    ...
    nivel_atividade = models.DecimalField(
        max_digits=3,
        decimal_places=2,
        null=True,
        blank=True,
        help_text="Fator de atividade física",
    )
    ...
    is_active = models.BooleanField(
        default=True, help_text="Se o plano está ativo", db_index=True
    )
...
class Meal(models.Model):
    ...
    diet = models.ForeignKey(
        Diet, on_delete=models.CASCADE, related_name="meals_rel"
    )  # Usando nome que não conflita
    name = models.CharField(
        max_length=100, help_text="Nome da refeição (Café, Almoço, etc)"
    )
...
class FoodItem(models.Model):
    ...
    quantity = models.DecimalField(
        max_digits=8, decimal_places=2, help_text="Quantidade em unidade"
    )
    unit = models.CharField(
        max_length=20, default="g", help_text="Unidade de medida (g, ml, und)"
    )

    # Valores nutricionais calculados (por porção)
    calories = models.DecimalField(
        max_digits=8, decimal_places=2, help_text="Calorias calculadas"
    )
    protein = models.DecimalField(
        max_digits=8, decimal_places=2, help_text="Proteínas em g"
    )
    carbs = models.DecimalField(
        max_digits=8, decimal_places=2, help_text="Carboidratos em g"
    )
    fats = models.DecimalField(
        max_digits=8, decimal_places=2, help_text="Gorduras em g"
    )
    fiber = models.DecimalField(
        max_digits=8, decimal_places=2, null=True, blank=True, help_text="Fibras em g"
    )

    # Referências opcionais às bases de dados
    taco_food = models.ForeignKey(
        AlimentoTACO,
...
# Substituições com equivalência nutricional baseada no macronutriente predominante
# =============================================================================


class FoodSubstitutionGroup(models.Model):
    """
    Grupo de substituição por macronutriente predominante.
    Ex: "Carboidratos Complexos", "Proteínas Magras", "Verduras Folhosas"
    """

    PREDOMINANT_NUTRIENT_CHOICES = [
        ("carbs", "Carboidrato"),
        ("protein", "Proteína"),
        ("fat", "Gordura"),
        ("fiber", "Fibra"),
    ...
    name = models.CharField(
        max_length=100,
        help_text="Nome do grupo de substituição (ex: Carboidratos Complexos)",
    )
    predominant_nutrient = models.CharField(
        max_length=20,
        choices=PREDOMINANT_NUTRIENT_CHOICES,
        help_text="Macronutriente predominante para cálculo de equivalência",
    )
    description = models.TextField(
        blank=True, null=True, help_text="Descrição do grupo"
    )
    ...
    class Meta:
        verbose_name = "Grupo de Substituição"
        verbose_name_plural = "Grupos de Substituição"

    def __str__(self):
        return f"{self.name} ({self.get_predominant_nutrient_display()})"


class FoodSubstitution(models.Model):
    """
    Substituição válida entre alimentos do mesmo grupo.
    Contém os dados nutricionais do substituto para cálculo de equivalência.
    """
    ...
    group = models.ForeignKey(
        FoodSubstitutionGroup, on_delete=models.CASCADE, related_name="substitutions"
    )

    # Alimento original
    original_source = models.CharField(
        max_length=50, choices=SOURCE_CHOICES, help_text="Fonte do alimento original"
    )
    original_food_id = models.CharField(
        max_length=100, help_text="ID do alimento original na fonte"
    )
    original_food_name = models.CharField(
        max_length=255, help_text="Nome do alimento original para exibição"
    )

    # Alimento substituto
    substitute_source = models.CharField(
        max_length=50, choices=SOURCE_CHOICES, help_text="Fonte do alimento substituto"
    )
    substitute_food_id = models.CharField(
        max_length=100, help_text="ID do alimento substituto na fonte"
    )
    substitute_food_name = models.CharField(
        max_length=255, help_text="Nome do alimento substituto para exibição"
    )

    # Macros por 100g do substituto (para cálculo dinâmico)
    substitute_calories_per_100g = models.FloatField(
        help_text="Calorias por 100g do substituto"
    )
    substitute_protein_per_100g = models.FloatField(
        help_text="Proteína por 100g do substituto"
    )
    substitute_carbs_per_100g = models.FloatField(
        help_text="Carboidratos por 100g do substituto"
    )
    substitute_fat_per_100g = models.FloatField(
        help_text="Gorduras por 100g do substituto"
    )
    substitute_fiber_per_100g = models.FloatField(
        default=0, help_text="Fibra por 100g do substituto"
    )

    # Flag para uso automático ou manual
    is_approved = models.BooleanField(
        default=True, help_text="Se True, a substituição está aprovada para uso"
    )
    order = models.IntegerField(
        default=0, help_text="Ordem de exibição na lista de substitutos"
    )
    ...
    class Meta:
        verbose_name = "Substituição de Alimento"
        verbose_name_plural = "Substituições de Alimentos"
        unique_together = [
            "original_source",
            "original_food_id",
            "substitute_source",
    ...
    def __str__(self):
        return f"{self.original_food_name} → {self.substitute_food_name}"
...
class MedidaCaseira(models.Model):
    ...
    nome = models.CharField(
        max_length=100,
        unique=True,
        help_text="Nome da medida caseira (ex: Colher de sopa)",
    )
    peso_medio_g = models.FloatField(
        null=True, blank=True, help_text="Peso médio em gramas"
    )
    categoria = models.CharField(
        max_length=20, choices=CATEGORIA_CHOICES, default="outro"
    )
    quantidade_amostras = models.IntegerField(
        default=0, help_text="Número de amostras usadas para calcular a média"
    )

    # Para líquidos
    eh_liquido = models.BooleanField(
        default=False, help_text="Se a medida é tipicamente para líquidos"
    )
    volume_ml = models.FloatField(
        null=True, blank=True, help_text="Volume em ml (para líquidos)"
    )
...
class AlimentoMedidaIBGE(models.Model):
    ...
    codigo_ibge = models.CharField(
        max_length=10, help_text="Código do alimento no IBGE"
    )
    nome_alimento = models.CharField(max_length=200, help_text="Nome do alimento")

    medida = models.ForeignKey(
        MedidaCaseira, on_delete=models.CASCADE, related_name="alimentos"
    )
    preparacao = models.CharField(
        max_length=20, choices=PREPARACAO_CHOICES, default="nao_aplica"
    )
    ...
    taco_ref = models.ForeignKey(
        AlimentoTACO,
        on_delete=models.SET_NULL,
        null=True,
        blank=True,
        related_name="medidas_ibge",
    )
    tbca_ref = models.ForeignKey(
        AlimentoTBCA,
        on_delete=models.SET_NULL,
        null=True,
        blank=True,
...
class DietViewLog(models.Model):
    ...
    patient = models.ForeignKey(
        PatientProfile, on_delete=models.CASCADE, related_name="diet_view_logs"
    )
...
class MealPreset(models.Model):
    ...
    description = models.TextField(
        blank=True, null=True, help_text="Descrição opcional do preset"
    )

    # Estrutura JSON para armazenar os alimentos do preset
    foods = models.JSONField(help_text="Lista de alimentos no preset", default=list)

    # Calorias totais calculadas para facilitar busca e filtragem
    total_calories = models.IntegerField(
        default=0, help_text="Calorias totais calculadas"
    )
    total_protein = models.DecimalField(
        max_digits=8, decimal_places=2, default=0, help_text="Proteínas totais em g"
    )
    total_carbs = models.DecimalField(
        max_digits=8, decimal_places=2, default=0, help_text="Carboidratos totais em g"
    )
    total_fats = models.DecimalField(
        max_digits=8, decimal_places=2, default=0, help_text="Gorduras totais em g"
    )

    is_active = models.BooleanField(default=True, help_text="Se o preset está ativo")
    is_public = models.BooleanField(
        default=False, help_text="Se o preset pode ser compartilhado com outros"
    )
...
class DefaultPreset(models.Model):
    ...
    is_active = models.BooleanField(
        default=True, help_text="Se o preset padrão está ativo"
    )
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    ...
    def __str__(self):
        return (
            f"{self.get_meal_type_display()} - {self.get_diet_type_display()} (Padrão)"
        )
...
class CustomFood(models.Model):
    ...
    nutritionist = models.ForeignKey(
        ...
        null=True,
        blank=True,
        help_text="Nutricionista proprietário do alimento (nulo para global)",
    )
    nome = models.CharField(max_length=200, help_text="Nome do alimento personalizado")
    grupo = models.CharField(
        max_length=100, blank=True, null=True, help_text="Grupo alimentar"
    )
    ...
    carboidrato_g = models.FloatField(
        default=0, help_text="Carboidrato em gramas por 100g"
    )
    fibra_g = models.FloatField(default=0, help_text="Fibra em gramas por 100g")
    ...
    unidade_caseira = models.CharField(
        ...
        blank=True,
        null=True,
        help_text="Ex: 1 Unidade, 1 Colher de sopa",
    )
    peso_unidade_caseira_g = models.FloatField(
        blank=True, null=True, help_text="Peso em gramas da unidade caseira"
    )
...
class FoodSubstitutionRule(models.Model):
    """
    Tabela global de substituições de alimentos.
    Define substituições válidas para cada tipo de dieta.
    """
    ...
    # Alimento original
    original_source = models.CharField(
        max_length=20, help_text="Fonte do alimento original (TACO, TBCA, USDA, IBGE)"
    )
    original_food_id = models.CharField(
        max_length=100, help_text="ID do alimento original na tabela de origem"
    )
    original_food_name = models.CharField(
        max_length=255, help_text="Nome do alimento original (cache para busca)"
    )

    # Alimento de substituição
    substitute_source = models.CharField(
        max_length=20, help_text="Fonte do alimento substituto"
    )
    substitute_food_id = models.CharField(
        max_length=100, help_text="ID do alimento substituto na tabela de origem"
    )
    substitute_food_name = models.CharField(
        max_length=255, help_text="Nome do alimento substituto (cache para busca)"
    )
    ...
    diet_type = models.CharField(
        max_length=20,
        choices=DIET_TYPE_CHOICES,
        help_text="Tipo de dieta para esta substituição",
    )

    # Critérios nutricionais
    nutrient_predominant = models.CharField(
        max_length=10,
        choices=NUTRIENT_PREDOMINANT,
        help_text="Macronutriente predominante (carb, protein, fat)",
    )
    similarity_score = models.DecimalField(
        max_digits=3, decimal_places=2, help_text="Similaridade nutricional (0.0 a 1.0)"
    )

    # Fator de conversão (para calcular quantidade)
    conversion_factor = models.DecimalField(
        max_digits=10,
        decimal_places=4,
        help_text="Multiplicador para ajustar quantidade baseada em macronutriente predominante",
    )

    # Quantidade padrão para este substituto (em gramas)
    suggested_quantity = models.DecimalField(
        max_digits=6,
        decimal_places=1,
        default=100,
        help_text="Quantidade padrão sugerida em gramas",
    )

    # Metadata
    priority = models.IntegerField(
        default=100, help_text="Prioridade de exibição (menor = mais prioritário)"
    )
    ...
    # Restrições alimentares do alimento substituto
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=["diet_type", "nutrient_predominant"]),
            models.Index(fields=["original_source", "original_food_id"]),
            models.Index(fields=["is_active"]),
            # Índices para filtros de restrição alimentar
            models.Index(
                fields=["is_vegan", "is_vegetarian", "is_gluten_free"],
                name="foodsub_dietary_idx",
            ),
            models.Index(
                fields=["original_source", "original_food_id", "is_vegan"],
                name="foodsub_orig_vegan_idx",
            ),
            models.Index(
                fields=["original_source", "original_food_id", "is_vegetarian"],
                name="foodsub_orig_veg_idx",
            ),
        ]

    def __str__(self):
        return f"{self.original_food_name} → {self.substitute_food_name} ({self.diet_type})"

    def save(self, *args, **kwargs):
        """Sanitizar campos de texto antes de salvar"""
        if self.original_food_name:
            self.original_food_name = sanitize_string(self.original_food_name)
        if self.substitute_food_name:
            self.substitute_food_name = sanitize_string(self.substitute_food_name)
        if self.notes:
            self.notes = sanitize_string(self.notes)
        super().save(*args, **kwargs)


class NutritionistSubstitutionFavorite(models.Model):
    """
    Substituições favoritas por nutricionista, alimento e tipo de dieta.
    Cada nutricionista pode ter suas preferências de substituição salvas.
    """
    ...
    nutritionist = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.CASCADE,
        related_name="substitution_favorites",
        help_text="Nutricionista que favoritou",
    )

    # Alimento original (contexto)
    original_source = models.CharField(
        max_length=20, help_text="Fonte do alimento original"
    )
    original_food_id = models.CharField(
        max_length=100, help_text="ID do alimento original"
    )
    original_food_name = models.CharField(
        max_length=255, help_text="Nome do alimento original"
    )

    # Tipo de dieta (contexto)
    diet_type = models.CharField(
        max_length=20,
        choices=FoodSubstitutionRule.DIET_TYPE_CHOICES,
        help_text="Tipo de dieta para este favorito",
    )

    # Substituição favorita
    substitute_source = models.CharField(
        max_length=20, help_text="Fonte do alimento substituto"
    )
    substitute_food_id = models.CharField(
        max_length=100, help_text="ID do alimento substituto"
    )
    substitute_food_name = models.CharField(
        max_length=255, help_text="Nome do alimento substituto"
    )
    ...
    # Similaridade (pode ser diferente da tabela principal)
    similarity_score = models.DecimalField(
        max_digits=3,
        decimal_places=2,
        default=0.80,
        help_text="Similaridade nutricional (0.0 a 1.0)",
    )

    # Prioridade pessoal (nutricionista pode ajustar)
    priority = models.IntegerField(
        default=10, help_text="Prioridade pessoal de exibição"
    )
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=["nutritionist", "diet_type", "original_food_id"]),
            models.Index(fields=["is_active"]),
        ]

    def __str__(self):
        return f"{self.nutritionist.email}: {self.original_food_name} → {self.substitute_food_name} ({self.diet_type})"
...
# =============================================================================
# MODELO UNIFICADO 2026 - INTELIGÊNCIA NUTRICIONAL (UUFT)
# =============================================================================

class UnifiedFood(models.Model):
    """
    O Coração do Sistema: Unifica TACO, TBCA e USDA em uma única taxonomia inteligente.
    Possibilita filtros biológicos (Grelhado, Cozido, Cru) e de Pureza (Staple vs Receita).
    """
    PREP_METHOD_CHOICES = [
        ('RAW', 'Cru/In Natura'),
        ('GRILLED', 'Grelhado'),
    ...
    PURITY_INDEX_CHOICES = [
        ('STAPLE', 'Comida de Verdade (Single Ingredient)'),
        ('COMPOSITE', 'Receita/Prato Composto (Multi-Ingredient)'),
        ('INDUSTRIAL', 'Ultraprocessado/Industrializado'),
    ]

    ANCHOR_MACRO_CHOICES = [
        ('PROTEIN', 'Proteína'),
        ('CARBS', 'Carboidrato'),
        ('FAT', 'Gordura'),
        ('FIBER', 'Fibra'),
        ('CALORIES', 'Equilíbrio Calórico (Mix)'),
    ]
    ...
    # Atributos de Inteligência 2026
    prep_method = models.CharField(max_length=20, choices=PREP_METHOD_CHOICES, default='OTHER')
    purity_index = models.CharField(max_length=20, choices=PURITY_INDEX_CHOICES, default='STAPLE')
    processing_level = models.IntegerField(default=1, help_text="Escala NOVA (1=Mínimo, 4=Ultraprocessado)")
    anchor_macro = models.CharField(max_length=20, choices=ANCHOR_MACRO_CHOICES, default='CALORIES')
    is_cooked = models.BooleanField(default=True)
    custom_category = models.CharField(max_length=50, null=True, blank=True, help_text="Clã Biológico (CEREAL, PROTEIN, etc)")
    
    # Rastreabilidade
    source_name = models.CharField(max_length=50, help_text="TACO, TBCA ou USDA")
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(fields=["prep_method"]),
            models.Index(fields=["purity_index"]),
            models.Index(fields=["anchor_macro"]),
        ]
