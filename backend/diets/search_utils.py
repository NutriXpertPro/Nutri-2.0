...
def calcular_score_radical(item_nome: str, q_tokens: list) -> float:
    ...
    # NOTA: Suplementos são EXCEÇÃO. Se o nome sugere suplemento, ignoramos a penalidade.
    is_suplemento = any(x in item_nome_lower for x in ['whey', 'bcaa', 'creatina', 'albumina', 'suplemento', 'hipercalorico', 'caseina'])
    
    if not is_suplemento:
        partes_alimentos = {
            'gema': 400.0,
            'clara': 400.0,
            'po': 300.0,
