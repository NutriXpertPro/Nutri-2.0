...
class DietSerializer(serializers.ModelSerializer):
    ...
    def validate_patient(self, value):
        """
        Valida se o paciente pertence ao nutricionista autenticado.
        Evita Broken Access Control (BAC).
        """
        request = self.context.get("request")
        if request and request.user:
            if value.nutritionist != request.user:
                raise serializers.ValidationError(
                    "Você não tem permissão para criar dietas para este paciente."
                )
        return value
