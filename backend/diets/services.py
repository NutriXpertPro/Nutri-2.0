...
def calculate_suggestion_service(
    ...
    original_group = (
        get_val(original_food, "grupo") or get_val(original_food, "categoria") or ""
    )
    ...
    def calc_rel(total, qty):
        ...
        try:
            t = float(total) if total not in (None, "", "nan", "NaN") else 0.0
            q = float(qty) if qty not in (None, "", "nan", "NaN") else 1.0
            if q <= 0:
                return t
            return (t * 100.0) / q
    ...
    p_base = (
        float(v_ptn)
        if v_ptn is not None
        else (safe_float(get_val(original_food, "proteina_g")) or 0.0)
    )
    c_base = (
        float(v_cho)
        if v_cho is not None
        else (safe_float(get_val(original_food, "carboidrato_g")) or 0.0)
    )
    l_base = (
        float(v_fat)
        if v_fat is not None
        else (safe_float(get_val(original_food, "lipidios_g")) or 0.0)
    )
    ...
    # 3. Motor de Substituição Perfeito V2026.1
    try:
        # Identificar método de preparo
        prep_method = identificar_preparo(food_instance_name)

        # Usar o sistema perfeito
        perfect_suggestions = sugerir_substituicoes_perfeito(
            original_nome=food_instance_name,
            original_grupo=group_name,
            original_quantidade=original_quantity,
            original_proteina_100g=p_base,
            original_carboidrato_100g=c_base,
            original_lipidio_100g=l_base,
            original_energia_100g=cal_base / max(original_quantity, 1) * 100
            if original_quantity > 0
            else cal_base,
            original_prep_method=prep_method,
            limite=20,
        )

        for res in perfect_suggestions:
            results.append(
                {
                    ...
                    "food": {
                        ...
                        "energia_kcal": res.calories_por_100g_substituto,
                    ...
                    "notes": f"Substituição Perfeita: Equivalência por {res.macronutriente_igualizado}",
                    "source": "AUTO",
                }
            )
    except Exception as e:
        logger.error(f"[SUGGEST ENGINE PERFEITO ERROR] {str(e)}", exc_info=True)
    ...
    fixed_rules = FoodSubstitutionRule.objects.filter(
        Q(original_food_name__icontains=search_term), is_active=True
    ...
    for rule in fixed_rules:
        ...
        if sub_data:
            ...
            sub_group = identificar_grupo_nutricional(
                sub_data["nome"], sub_data.get("grupo", "")
            )
            res_sub = calcular_substituicao(
                current_nutri, sub_item, group_name or sub_group, original_quantity
            )
    ...
    return (
        {"substitutions": cleaned_results[:limite], "engine_status": engine_status},
        group_name or "mixed",
        200,
    )
