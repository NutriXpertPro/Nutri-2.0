...
def send_welcome_email_task(self, user_id, nutritionist_name):
    ...
    """
    # Se o 'self' for None ou não tiver o atributo 'request', significa que foi chamado
    # diretamente ou em modo de fallback síncrono.
    is_sync = not hasattr(self, 'request') or self.request is None
    
    try:
        if is_sync:
            file_log(f"[TASK-SYNC] Executando sincronamente para user_id: {user_id}")
        else:
            file_log(f"[TASK-ASYNC] Iniciando para user_id: {user_id}")
...
Como nutricionista responsável pelo seu acompanhamento, tenho total confiança que juntos alcançaremos excelentes resultados conforme seu objetivo.
...
"""
        ...
        file_log(error_msg)
        # Reagendar tarefa em caso de erro (ex: falha na conexão SMTP) apenas se não for síncrono
        if not is_sync:
