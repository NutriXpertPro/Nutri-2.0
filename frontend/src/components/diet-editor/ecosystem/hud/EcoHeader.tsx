...
export function EcoHeader() {
    ...
    // Listen for preview events from other components
    React.useEffect(() => {
        const handleOpenPreview = () => {
            setShowPDFModal(true);
        };
        window.addEventListener('open-pdf-preview', handleOpenPreview);
        return () => window.removeEventListener('open-pdf-preview', handleOpenPreview);
    }, []);
    ...
    const handleSendApp = async () => {
        ...
        if (!hasContent) {
            toast({
                title: "Dieta vazia",
                description: "Adicione alimentos à dieta antes de enviar para o app.",
                variant: "destructive"
            })
            return
        }
        ...
        try {
            // Salvar a dieta (já marca como is_active: true no backend)
            await saveDiet()
            ...
            toast({
                ...
                description: error?.message || "Não foi possível enviar a dieta para o app. Tente novamente.",
                variant: "destructive"
            })
        }
    }
    ...
    const currentTotals = React.useMemo(() => {
        ...
        if (!activeTab || activeTab === 'diet') {
            return workspaceMeals.reduce((acc, meal) => {
                meal.foods.forEach(food => {
                    ...
                    const ptn = food.ptn * ratio
                    const cho = food.cho * ratio
                    const fat = food.fat * ratio
                    const fib = (food.fib || 0) * ratio

                    acc.protein += ptn
                    acc.carbs += cho
                    acc.fats += fat
                    acc.fiber += fib
                    acc.calories += (ptn * 4 + cho * 4 + fat * 9)
                })
                return acc
        ...
        return meals.reduce((acc, meal) => {
            meal.items.forEach(item => {
                const ptn = Number(item.protein)
                const cho = Number(item.carbs)
                const fat = Number(item.fats)
                const fib = Number((item as any).fiber || 0)
    ...
    const getMacroData = (current: number, target: number, label: string) => {
        ...
        const targetPerKg = (target / weight).toFixed(1)
        ...
        return { current, target, perKg, targetPerKg, diff, progress, statusColor, label }
    }
                    ...
                    <div className="absolute left-[54%] -translate-x-1/2 -top-4 flex items-center gap-6">
                        ...
                        <div className="flex flex-col">
                            ...
                            <div className="flex items-baseline gap-2">
                                ...
                                <span className="text-xs lg:text-sm text-muted-foreground opacity-40 font-medium">/ {targetCalories}</span>
                            </div>
        ...
        <div className="flex flex-col gap-2 group">
            <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-1">
                    <span className={cn("px-1.5 py-0.5 rounded-md text-[9px] tracking-tighter font-bold", data.diff > 0 ? 'bg-red-500/10 text-red-500' : 'bg-emerald-500/10 text-emerald-500')}>
                        {data.diff > 0 ? 'Faltam ' : ''}{Math.abs(data.diff)}g
                    </span>
                </div>
            </div>
            <div className="flex items-end justify-between -mt-1">
                ...
                <div className="flex flex-col items-end">
                    <span className="text-[10px] text-muted-foreground tabular-nums opacity-60 leading-none">{data.perKg}g/kg</span>
                    <span className="text-[8px] text-muted-foreground/30 tabular-nums leading-none mt-1">Meta: {data.targetPerKg}</span>
                </div>
            </div>
...
function CircularMacroChart({ totals, targets, targetCalories, hideText }: any) {
    ...
    // CORREÇÃO: Porcentagem calórica, não por gramas
    const ptnKcal = totals.protein * 4
    const choKcal = totals.carbs * 4
    const fatKcal = totals.fats * 9
    const totalKcal = ptnKcal + choKcal + fatKcal

    const safeTotal = totalKcal || 1
    const proteinPct = (ptnKcal / safeTotal) * 100
    const carbsPct = (choKcal / safeTotal) * 100
    const fatsPct = (fatKcal / safeTotal) ? (fatKcal / safeTotal) * 100 : 0
