    ...
    <div className="w-full h-full flex flex-col bg-background text-foreground min-h-0">
      ...
      <div className="p-4 bg-background">
        <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as 'presets' | 'defaults')} className="w-full">
          <TabsList className="w-full grid grid-cols-2 gap-4 bg-transparent h-14 p-0 border-none">
            <TabsTrigger
              value="presets"
              className="h-full flex items-center gap-3 justify-center rounded-xl border border-border/50 bg-muted/30 shadow-sm transition-all data-[state=inactive]:border-primary/40 data-[state=active]:bg-primary data-[state=active]:text-primary-foreground data-[state=active]:border-primary font-normal group"
            >
              <div className="w-8 h-8 rounded-lg bg-primary/10 flex items-center justify-center group-data-[state=active]:bg-white group-data-[state=active]:text-primary transition-all duration-300">
            ...
            <TabsTrigger
              ...
              className="h-full flex items-center gap-3 justify-center rounded-xl border border-border/50 bg-muted/30 shadow-sm transition-all data-[state=inactive]:border-primary/40 data-[state=active]:bg-primary data-[state=active]:text-primary-foreground data-[state=active]:border-primary font-normal group"
            >
              <div className="w-8 h-8 rounded-lg bg-yellow-500/10 flex items-center justify-center group-data-[state=active]:bg-white transition-all duration-300">
...
function MealPresetsManagerContent({
  ...
  const handleSavePreset = async () => {
    try {
      const currentPreset = isCreating ? newPreset : editingPreset;
      // Verificar se currentPreset existe
      if (!currentPreset) {
        toast.error("Erro interno", { description: "Nenhum preset selecionado para salvar." });
        return;
      }

      // Gerar nome automaticamente baseado no tipo de refeição e dieta
      const mealLabel = MEAL_TYPES.find(m => m.id === currentPreset.meal_type)?.label || currentPreset.meal_type;
      const dietLabel = DIET_TYPES.find(d => d.id === currentPreset.diet_type)?.label || currentPreset.diet_type;
      const autoGeneratedName = `${mealLabel} - ${dietLabel}`;

      // Atualizar o nome do preset automaticamente
      if (isCreating) {
        setNewPreset({ ...newPreset, name: autoGeneratedName });
      } else if (editingPreset) {
        setEditingPreset({ ...editingPreset, name: autoGeneratedName });
      }

      // Usar o nome gerado no currentPreset
      currentPreset.name = autoGeneratedName;

      if (!currentPreset.foods?.length || currentPreset.foods.length === 0) {
        toast.warning("Adicione alimentos ao preset", {
          description: "O preset precisa ter pelo menos um alimento."
        });
        return;
      }
      ...
      if (isCreating) {
        ...
        console.log('[DEBUG handleSavePreset] Chamando createMealPreset com payload:', JSON.stringify(payload, null, 2));
        await createMealPreset(payload);
        ...
        console.log('[DEBUG handleSavePreset] Chamando updateMealPreset com id:', editingPreset.id, 'payload:', JSON.stringify(payload, null, 2));
        await updateMealPreset(editingPreset.id, payload);
      ...
      if (error?.response?.data) {
        if (typeof error.response.data === 'string') {
          ...
          // Extrair erros de campos específicos (DRF style)
              ...
              <div className="mb-3">
                ...
                <div className="flex flex-wrap gap-2">
                  {MEAL_TYPES.map(meal => (
                    <button
                      key={meal.id}
                      className={`px-3 py-1.5 rounded-lg text-xs font-normal transition-colors border ${selectedMealType === meal.id
                        ? 'bg-primary text-primary-foreground border-primary shadow-sm'
                        : 'bg-muted hover:bg-muted/80 text-foreground border-primary/60'
                        }`}
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {DIET_TYPES.map(diet => (
                    <button
                      key={diet.id}
                      className={`px-3 py-1.5 rounded-lg text-xs font-normal transition-colors border ${selectedDietType === diet.id
                        ? 'bg-primary text-primary-foreground border-primary shadow-sm'
                        : 'bg-muted hover:bg-muted/80 text-foreground border-primary/60'
                        }`}
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div className="flex-1 min-w-0">
                            <h4 className="font-normal text-sm text-foreground truncate">
                              ...
                              <span className={`font-normal ${preset.diet_type === 'low_carb' ? 'text-amber-500' :
