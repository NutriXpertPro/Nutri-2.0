...
const WhatsAppConversationsList: React.FC<WhatsAppConversationsListProps> = ({
  ...
  const handleSelectPatient = async (patient: Patient) => {
    ...
    // @ts-ignore - patient may have user_id from API
    ...
    if (existingConversation) {
      ...
      try {
        ...
        if (newConv.participants) {
          newConv.participants = newConv.participants.map((p: UserType) => {
            if (String(p.id) === targetUserId) {
              return {
                ...p,
                name: p.name || patient.name,
                avatar: p.avatar || patient.avatar
              };
            }
            return p;
          });
      ...
      try {
        ...
        if (axios.isAxiosError(error) && error.response?.status === 404) {
          try {
            ...
            const conversationsData = response.data.map((conv: Conversation) => ({
              ...conv,
              last_message: conv.messages && conv.messages.length > 0 ? conv.messages[conv.messages.length - 1].content : '',
              last_message_time: conv.messages && conv.messages.length > 0 ? conv.messages[conv.messages.length - 1].timestamp : null,
              unread_count: 0 // Para simplificar, inicializar como 0
            }));
            setConversations(conversationsData);
          } catch (fallbackError) {
            if (!silent) console.error(fallbackError); // Error silenciado
          }
        } else {
  ...
  useEffect(() => {
    ...
    const fetchMessages = async (silent = false) => {
      try {
        ...
        const uniqueMessages = Array.from(new Map(newMessages.map((m: Message) => [m.id, m])).values());
