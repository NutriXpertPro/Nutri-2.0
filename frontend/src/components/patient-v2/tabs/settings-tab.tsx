import { Bell, Palette, Shield, LogOut, ChevronRight, Loader2, AlertCircle } from "lucide-react"
...
export function SettingsTab({ }: { onBack?: () => void }) {
    ...
    // Sync settings from API (Only once or when actually changed externally)
    useEffect(() => {
        if (settings?.notifications && !isInitialized.current) {
    ...
    useEffect(() => {
        ...
        const timeoutId = setTimeout(() => {
            // Check if values are different from current settings to avoid unnecessary calls
            if (settings?.notifications) {
                const currentSettings = settings.notifications
                const hasChanges =
                    currentSettings.email !== mealReminders ||
                    currentSettings.reminderTime?.toString() !== mealAntecedence ||
                    currentSettings.push !== waterReminders ||
                    currentSettings.snooze !== snoozeEnabled
                ...
                if (hasChanges) {
                    updateSettings({
                        notifications: {
                            email: mealReminders,
                            push: waterReminders,
                            sms: currentSettings.sms || false,
                            reminderTime: parseInt(mealAntecedence),
                            snooze: snoozeEnabled
                        }
                    }).catch(() => {
                        // Silent error catch for auto-save
                    })
                }
            }
        }, 1000)
    ...
    }, [mealReminders, mealAntecedence, waterReminders, snoozeEnabled, settings, updateSettings])
