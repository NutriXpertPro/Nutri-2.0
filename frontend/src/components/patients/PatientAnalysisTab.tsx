...
import { Scale, Plus, Loader2, MessageSquare, Trash2, Filter, Activity, Zap } from "lucide-react"
...
// Data de fallback constante para evitar funções impuras no render
        ...
        <div className="space-y-8 pb-12">
            ...
            <div className="p-4 bg-background/40 rounded-xl border border-border/10 flex items-center justify-between">
                <div className="flex items-center gap-4">
                    <Avatar className="w-12 h-12 border border-background shadow-sm">
                        <AvatarImage src={patient?.avatar} alt={patient?.name || "Paciente"} className="object-cover" />
                            ...
                            {patient?.notes && patient.notes.length > 0 ? (
                                <div className="space-y-6 relative before:absolute before:inset-y-0 before:left-[15px] before:w-[1px] before:bg-border/40">
                                    ...
                                    {patient.notes.map((note: any, idx: number) => {
                                        const noteDate = note.created_at ? new Date(note.created_at) : FALLBACK_DATE
                                        return (
