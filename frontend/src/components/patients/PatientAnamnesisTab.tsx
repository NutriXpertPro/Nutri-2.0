...
export function PatientAnamnesisTab() {
    ...
    useEffect(() => {
        if (anamnesisViewMode === 'view-responses') {
            setView(prev => prev !== 'VIEW_RESPONSES' ? 'VIEW_RESPONSES' : prev)
        } else if (anamnesisViewMode === 'fill-standard') {
            setView(prev => prev !== 'FILL_STANDARD' ? 'FILL_STANDARD' : prev)
        } else if (anamnesisViewMode === 'fill-custom') {
            setView(prev => prev !== 'FILL_CUSTOM' ? 'FILL_CUSTOM' : prev)
        }
    }, [anamnesisViewMode]);
    ...
    const handleSaveTemplate = (templateData: Omit<AnamnesisTemplate, 'id'>) => {
    ...
    const saveStandardMutation = useMutation({
        mutationFn: (data: StandardAnamnesisData) => anamnesisService.saveStandardAnamnesis(patientId, data),
        onSuccess: () => {
    ...
    const handleSaveAnamnesis = async (data: StandardAnamnesisData) => {
                                ...
                                <div className="space-y-6">
                                    <div className="flex items-center gap-5">
                                        <div className="bg-white dark:bg-zinc-900 p-3 rounded-2xl shadow-xl shadow-black/5 ring-1 ring-black/5">
                                            <Image src="/logo.png" alt="Nutri Xpert Pro" width={160} height={40} className="h-10 w-auto" />
                                        </div>
                                        <div>
...
function ClinicalSection({ title, icon: Icon, children }: { title: string, icon: React.ComponentType<{ className?: string }>, children: React.ReactNode }) {
    return (
    ...
    return (
        <div className="flex items-center gap-3 group select-none cursor-default">
            ...
            <div className="relative">
                ...
                <Image src="/logo.png" alt="Icon" width={40} height={40} className="h-10 w-auto relative z-10 drop-shadow-md transition-all duration-500 group-hover:scale-110" />
            </div>
            {/* Logo Text */}
