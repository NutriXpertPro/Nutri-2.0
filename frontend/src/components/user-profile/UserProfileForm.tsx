...
const profileFormSchema = z.object({
  ...
  // Permite FileList ou null/undefined
  profile_picture: z.custom<FileList | null | undefined>().optional(),
});
...
export function UserProfileForm() {
  ...
  useEffect(() => {
    if (userProfile) {
      // Reset form values only if they differ significantly or on initial load
      // Using reset inside useEffect is standard for populating forms from API data
      form.reset({
      ...
      // Handle preview image state update to avoid redundant renders
      if (userProfile.profile_picture && userProfile.profile_picture !== previewImage) {
        setPreviewImage(userProfile.profile_picture);
      } else if (!userProfile.profile_picture && previewImage !== null) {
        setPreviewImage(null);
      }
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [userProfile, form]); // Removed previewImage from deps to avoid loop
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      form.setValue("profile_picture", event.target.files, { shouldValidate: true, shouldDirty: true });
    }
  };
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex flex-col gap-2">
                    <Input
                      ...
                      accept="image/*"
                      onChange={(e) => {
                        // Not directly passing files to onChange because controlled inputs work differently with files
                        // but we trigger the field change to mark dirty/valid
