...
const dietService = {
    ...
    create: async (dietData: Omit<Diet, 'id'>) => {
        const response = await api.post<Diet>('diets/', dietData);
        return response.data;
    },

    // --- Meal Presets ---
    getMealPresets: async (): Promise<any[]> => {
        const response = await api.get('diets/meal-presets/');
        return response.data;
    },

    createMealPreset: async (presetData: Omit<any, 'id'>) => {
        const response = await api.post('diets/meal-presets/', presetData);
        return response.data;
    },

    updateMealPreset: async (id: number, presetData: Partial<any>) => {
    ...
    createDefaultPreset: async (data: Omit<any, 'id'>) => {
        const response = await api.post('diets/default-presets/', data);
        return response.data;
    },

    updateDefaultPreset: async (id: number, data: Partial<any>) => {
